# PR #10356: Move disruptive MultiKueue tests to separate suite

**Summary**: Move disruptive MultiKueue tests to separate suite

**Sources**: https://github.com/kubernetes-sigs/kueue/pull/10356

**Last updated**: 2026-04-13T12:15:11Z
...
- **Branch**: `feat/10200-separate-disruptive-e2e-mk` → `main`
- **Created**: 2026-04-09T12:26:03Z
- **Updated**: 2026-04-13T12:15:11Z
- **Merged**: 2026-04-13T08:48:09Z
- **Closed**: 2026-04-13T08:48:09Z
- **Labels**: `lgtm`, `size/XXL`, `release-note-none`, `approved`, `kind/cleanup`, `cncf-cla: yes`, `area/testing`
- **Assignees**: [@mimowo](https://github.com/mimowo)
- **Requested reviewers**: [@mimowo](https://github.com/mimowo), [@mbobrovskyi](https://github.com/mbobrovskyi)
- **Changed files**: 9   **+1120 / -684**
...
Separate disruptive tests to allow for parallel run of other tests.
...
### Comment by [@netlify[bot]](https://github.com/apps/netlify) — 2026-04-09T12:26:11Z
...
|  Name | Link |
|:-:|------------------------|
|<span aria-hidden="true">🔨</span> Latest commit | c43382fc4ff1764133682607ac247e5adac24a24 |
|<span aria-hidden="true">🔍</span> Latest deploy log | https://app.netlify.com/projects/kubernetes-sigs-kueue/deploys/69dca3fa7559a8000741d2b2 |

### Comment by [@mimowo](https://github.com/mimowo) — 2026-04-10T07:03:35Z

cc @kshalot @olekzabl

### Comment by [@k8s-ci-robot](https://github.com/k8s-ci-robot) — 2026-04-13T07:21:31Z

LGTM label has been added.  <details>Git tree hash: 8671e81ba8c8a542ea83aeacc62acbdd60430fcc</details>

### Comment by [@k8s-ci-robot](https://github.com/k8s-ci-robot) — 2026-04-13T07:21:33Z

[APPROVALNOTIFIER] This PR is **APPROVED**

This pull-request has been approved by: *<a href="https://github.com/kubernetes-sigs/kueue/pull/10356#pullrequestreview-4096991204" title="Approved">mimowo</a>*, *<a href="https://github.com/kubernetes-sigs/kueue/pull/10356#" title="Author self-approved">mszadkow</a>*, *<a href="https://github.com/kubernetes-sigs/kueue/pull/10356#pullrequestreview-4088051467" title="Approved">olekzabl</a>*
...
### Comment by [@mimowo](https://github.com/mimowo) — 2026-04-13T08:09:17Z

/lgtm

### Comment by [@k8s-ci-robot](https://github.com/k8s-ci-robot) — 2026-04-13T08:09:24Z

LGTM label has been added.  <details>Git tree hash: 31e3aaeb377f48f7b35b8e76c6a7d9138023066b</details>

### Comment by [@mimowo](https://github.com/mimowo) — 2026-04-13T12:14:28Z

seems like the robot cannot do the CPs, @mszadkow could you prepare manually?

### Comment by [@mimowo](https://github.com/mimowo) — 2026-04-13T12:15:11Z

Please later also follow up with the test-infra changes.

## Reviews

### Review by [@mimowo](https://github.com/mimowo) (commented) — 2026-04-10T07:03:19Z

- **test/e2e/multikueue/e2e_test.go:1789** by [@mimowo](https://github.com/mimowo) — 2026-04-10T07:03:19Z
  IIUC this is the pre-requistite step to enable running MK tests in parallel, those which aren't disruptive.
  So, there will be a follow up work to use the namespace-suffix for the Cluster-scoped resources here, right @mszadkow ?

### Review by [@olekzabl](https://github.com/olekzabl) (approved) — 2026-04-10T07:48:12Z

- **Makefile-test.mk:205** by [@olekzabl](https://github.com/olekzabl) — 2026-04-10T07:24:41Z
```diff
@@ -198,6 +201,10 @@ test-e2e-dra: setup-e2e-env run-test-e2e-dra-$(E2E_KIND_VERSION:kindest/node:v%=
...
+.PHONY: test-e2e-multikueue-disruptive
+test-e2e-multikueue-disruptive: setup-e2e-env run-test-e2e-multikueue-disruptive-$(E2E_KIND_VERSION:kindest/node:v%=%)
```

  Duplicate of lines 161-162?

- **test/e2e/multikueue-sequential/e2e_test.go:913** by [@olekzabl](https://github.com/olekzabl) — 2026-04-10T07:32:54Z
```diff
@@ -0,0 +1,917 @@
+/*
...
+)
+
+var _ = ginkgo.Describe("MultiKueue Disruptive", func() {
+	var (
...
+			waitForJobAdmitted(wlLookupKey, multiKueueAc.Name, workerCluster2.Name)
+
...
+	})
+
+	ginkgo.When("ClusterProfile no plugins present", ginkgo.Ordered, func() {
+		var (
...
+	})
+
+	ginkgo.When("ClusterProfile plugins present", ginkgo.Ordered, func() {
+		const (
...
+	})
+
+	ginkgo.When("Worker cluster is unreliable", ginkgo.Ordered, func() {
+		ginkgo.It("Should update the cluster status to reflect the connection state", func() {
+			regularFlavor := utiltestingapi.MakeResourceFlavor("default-regular").Obj()
+			util.MustCreate(ctx, k8sWorker1Client, regularFlavor)
+
+			worker1Cq2 := utiltestingapi.MakeClusterQueue("q2").
+				ResourceGroup(
+					*utiltestingapi.MakeFlavorQuotas(regularFlavor.Name).
+						Resource(corev1.ResourceCPU, "2").
+						Resource(corev1.ResourceMemory, "1G").
+						Obj(),
+				).
...
+				util.ExpectObjectToBeDeletedWithTimeout(ctx, k8sWorker1Client, regularFlavor, true, util.MediumTimeout)
+			})
+		})
+	})
+
...
```
  ...
  AFAICS these helpers got duplicated - should we move them to some shared util file?

- **test/e2e/multikueue-disruptive/e2e_test.go:638** by [@olekzabl](https://github.com/olekzabl) — 2026-04-10T07:40:10Z
```diff
@@ -0,0 +1,917 @@
+/*
+Copyright The Kubernetes Authors.
...
```
  ...
  This part is not a pure move - you introduced `regularFlavor` (and also changed the test name).
  And - if I'm not mistaken - these are your additions, not some older version sneaking in.
  Out of curiosity, why?

- **test/e2e/config/multikueue-disruptive/controller_manager_config.yaml:24** by [@olekzabl](https://github.com/olekzabl) — 2026-04-10T07:46:59Z
```diff
@@ -0,0 +1,24 @@
...
### Review by [@mszadkow](https://github.com/mszadkow) (commented) — 2026-04-10T10:00:13Z

- **test/e2e/multikueue-sequential/e2e_test.go:913** by [@mszadkow](https://github.com/mszadkow) — 2026-04-10T10:00:13Z
```diff
@@ -0,0 +1,917 @@
+/*
+Copyright The Kubernetes Authors.
...
```
  ...
  Yes, I plan a follow-up  or maybe I could provide prep for that functions, I didn't want to mix those.

### Review by [@mszadkow](https://github.com/mszadkow) (commented) — 2026-04-10T10:01:59Z

- **test/e2e/multikueue/e2e_test.go:1789** by [@mszadkow](https://github.com/mszadkow) — 2026-04-10T10:01:59Z
  Yes, that's correct

### Review by [@kshalot](https://github.com/kshalot) (commented) — 2026-04-10T10:03:57Z

- **Makefile-test.mk:162** by [@kshalot](https://github.com/kshalot) — 2026-04-10T10:00:43Z
```diff
@@ -153,7 +153,13 @@ test-multikueue-e2e-parallel-builds:
...
+test-multikueue-e2e: test-multikueue-e2e-main test-multikueue-e2e-disruptive
+
...
+
+.PHONY: test-multikueue-e2e-disruptive
+test-multikueue-e2e-disruptive: setup-e2e-env run-test-e2e-multikueue-disruptive-$(E2E_KIND_VERSION:kindest/node:v%=%)
```

  I realize that the name "disruptive" comes from the original issue, but wouldn't "sequential" be more self explanatory?
  
  When I'm adding a test, I think it's more natural for me to notice that my test needs to run sequentially rather than considering them to be somehow disruptive. It's also not clear what is being disrupted by the tests without prior context.
  
  WDYT?

### Review by [@mszadkow](https://github.com/mszadkow) (commented) — 2026-04-10T10:05:51Z

- **Makefile-test.mk:162** by [@mszadkow](https://github.com/mszadkow) — 2026-04-10T10:05:51Z
```diff
@@ -153,7 +153,13 @@ test-multikueue-e2e-parallel-builds:
 	$(MAKE) -j2 kind-ray-project-mini-image-build kind-secretreader-plugin-image-build
...
```
  ...
  Thanks, I couldn't find a better name, this sounds much more descriptive, without introducing this "worrying" scent ;)

### Review by [@mimowo](https://github.com/mimowo) (commented) — 2026-04-10T10:11:53Z

- **Makefile-test.mk:162** by [@mimowo](https://github.com/mimowo) — 2026-04-10T10:11:53Z
```diff
@@ -153,7 +153,13 @@ test-multikueue-e2e-parallel-builds:
 	$(MAKE) -j2 kind-ray-project-mini-image-build kind-secretreader-plugin-image-build
...
```
  ...
  Yes, I like sequential as well. Maybe in the future we will also have tests which need to run sequentiall but are not disruptive, for example check some metrics which are global and can be affected by any test.

### Review by [@mszadkow](https://github.com/mszadkow) (commented) — 2026-04-10T10:14:37Z

- **test/e2e/multikueue-disruptive/e2e_test.go:638** by [@mszadkow](https://github.com/mszadkow) — 2026-04-10T10:14:37Z
```diff
@@ -0,0 +1,917 @@
+/*
+Copyright The Kubernetes Authors.
...
```
  ...
  My bad, I have mistakenly added the flavor here.
  The name is a different thing, that left after initial approach with labels, cuz I wanted to group them under one test container.
  I will amke sure there are no more name changes, thanks for spotting this.

### Review by [@olekzabl](https://github.com/olekzabl) (commented) — 2026-04-10T10:40:59Z

- **Makefile-test.mk:162** by [@olekzabl](https://github.com/olekzabl) — 2026-04-10T10:40:58Z
```diff
@@ -153,7 +153,13 @@ test-multikueue-e2e-parallel-builds:
 	$(MAKE) -j2 kind-ray-project-mini-image-build kind-secretreader-plugin-image-build
...
```
  ...
  For me, "sequential" also would be likely unclear without prior context. I think I'd then imagine some "sequence" _inside_ the test, while (IIUC) here we're talking about _external_ arrangement, whether a particular test can execute in parallel with some others.
  
  That said, I'm not contesting the name, I like it. (Especially in contrast with "parallel", it just feels natural).
  I'm just thinking, maybe you could add some comment (e.g. on top of `multikueue-sequential/e2e_test.go`, or wherever feels more appropriate) to clarify the meaning?

### Review by [@kshalot](https://github.com/kshalot) (commented) — 2026-04-10T14:53:03Z

- **Makefile-test.mk:162** by [@kshalot](https://github.com/kshalot) — 2026-04-10T14:53:03Z
```diff
@@ -153,7 +153,13 @@ test-multikueue-e2e-parallel-builds:
 	$(MAKE) -j2 kind-ray-project-mini-image-build kind-secretreader-plugin-image-build
...
```
  ...
  We can also consider being more verbose and use `sequentially-executed`, but I'm not sure it's really that much better. I agree with the remark about adding a comment.

### Review by [@mimowo](https://github.com/mimowo) (commented) — 2026-04-13T07:20:41Z

- **Makefile-test.mk:162** by [@mimowo](https://github.com/mimowo) — 2026-04-13T07:20:40Z
```diff
@@ -153,7 +153,13 @@ test-multikueue-e2e-parallel-builds:
 	$(MAKE) -j2 kind-ray-project-mini-image-build kind-secretreader-plugin-image-build
...
```
  ...
  I think sequential is preferred as it is short. 
  
  btw, we also have similar pattern for singlecluster, we call them customconfigs, but I think we could also rename to sequential, because there also some tests aren't really testing different configurations, but for example cert management. So "customconfigs", similarly as "disruptive" was the idea to name them after the main use case, but since the primary use case is not the only after a while, I think it makes sense to call more generically, reflecting what is actually happening. The name "sequential" would also fit both "multikueue" and "singlecluster" test.

### Review by [@mimowo](https://github.com/mimowo) (commented) — 2026-04-13T07:21:23Z
