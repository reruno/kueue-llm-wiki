...
**Last updated**: 2026-06-05T12:24:33Z
...
### Comment by [@amy](https://github.com/amy) — 2026-05-20T22:05:58Z
...
### Review by [@olekzabl](https://github.com/olekzabl) (commented) — 2026-05-20T20:36:11Z
...
- **keps/8692-composable-multikueue-dispatcher/README.md:408** by [@olekzabl](https://github.com/olekzabl) — 2026-05-20T13:30:57Z
```diff
@@ -0,0 +1,791 @@
+# KEP-8692: Composable MultiKueue Dispatcher
+
...
- **keps/8692-composable-multikueue-dispatcher/README.md:727** by [@olekzabl](https://github.com/olekzabl) — 2026-05-20T14:00:08Z
```diff
@@ -0,0 +1,791 @@
+# KEP-8692: Composable MultiKueue Dispatcher
+
...
```
  ...
  Is it _filter_ ("required") vs. _score_ ("preferred")? Or sth more?

- **keps/8692-composable-multikueue-dispatcher/README.md:324** by [@olekzabl](https://github.com/olekzabl) — 2026-05-20T14:04:32Z
```diff
@@ -0,0 +1,791 @@
+# KEP-8692: Composable MultiKueue Dispatcher
+
...
- **keps/8692-composable-multikueue-dispatcher/README.md:100** by [@olekzabl](https://github.com/olekzabl) — 2026-05-20T19:11:13Z
```diff
@@ -0,0 +1,791 @@
+# KEP-8692: Composable MultiKueue Dispatcher
+
...
```
  ...
  I imagine we could _strictly reject_ a workload if its requests exceed _worker (whole) capacity_. 
  "Unreserved capacity" feels like valuable information - though ideally only for _soft scoring_.

- **keps/8692-composable-multikueue-dispatcher/README.md:486** by [@olekzabl](https://github.com/olekzabl) — 2026-05-20T20:24:13Z
```diff
@@ -0,0 +1,791 @@
+# KEP-8692: Composable MultiKueue Dispatcher
+
...
```
  ...
  3. _If the remote cluster later evicts the workload, the eviction propagates back to the manager_
     You say this for `BestEffort` - but why wouldn't it hold also for `MustBeAdmitted`?
  
  4. _If the remote does not admit within `workerLostTimeout`, the dispatcher retries with the next-best cluster._
  ...
  5. You mention `workerLostTimeout` but I'm feeling it doesn't fit in this story.
     * IIUC the intent behind `workerLostTimeout` is to handle cases when we _lost connection_ to a worker cluster on which the remote workload _had already started_; see [this comment](https://github.com/kubernetes-sigs/kueue/blob/4e440ec3f781c2e86c0b0041d89589995ff80f8e/pkg/controller/admissionchecks/multikueue/workload.go#L505-L506) and the description of #1681.
       In that case, we want to _speculatively assume_ for some time that the workload is still running there - to avoid re-dispatching which could later turn out to have been wasteful.
     * The case in this KEP seems very different. Here (IIUC) we still _can see_ the worker cluster but the workload _failed to start_ on it.
  ...
  6. I can't help the feeling that the choice between `BestEffort` / `MustBeAdmitted`:
     * will not affect the _actual status_ (running / not running) of the workload on the 1st selected worker, or the timeline of that status
  ...
  7. Historically, the early MultiKueue behavior was "mark the workload `Admitted` on the manager as soon as it got `QuotaReserved` on some worker" - but then, it was considered imperfect and evolved towards "let manager-side admission follow worker-side admission" (#8585).
...
- **keps/8692-composable-multikueue-dispatcher/README.md:781** by [@olekzabl](https://github.com/olekzabl) — 2026-05-20T20:27:52Z
```diff
@@ -0,0 +1,791 @@
+# KEP-8692: Composable MultiKueue Dispatcher
+
...
```
  ...
  Cf. [KEP-9270](https://github.com/kubernetes-sigs/kueue/pull/10877) which proposes a similar thing for the incremental dispatcher.
...
### Review by [@olekzabl](https://github.com/olekzabl) (commented) — 2026-06-05T12:24:33Z

- **keps/8692-composable-multikueue-dispatcher/README.md:102** by [@olekzabl](https://github.com/olekzabl) — 2026-06-05T12:24:33Z
```diff
@@ -0,0 +1,791 @@
+# KEP-8692: Composable MultiKueue Dispatcher
+
...
```
  ...
  I think [#10105](https://github.com/kubernetes-sigs/kueue/issues/10105) (which we kind of shelved for now) is actually **not a blocker** here.
  
  * The key blockers for the progress of [#10105](https://github.com/kubernetes-sigs/kueue/issues/10105) are:
    * how to efficiently measure the "given manager CQ -> given worker CQ used volume" (which in general requires looking at individual workloads due to many-to-many CQ-to-CQ relation)
    * how to nicely surface the resource stats in the API
  
    which are both *irrelevant* for the purposes of this KEP.
  
  * On the other hand, what this KEP needs is the awareness of total / unused workers capacity on the manager side, possibly broken down per flavor.
    That I'd say is *almost* achieved in [#11141](https://github.com/kubernetes-sigs/kueue/pull/11141); specifically:
    * manager CQ is now aware of worker CQ quotas
    * the missing bits are awareness of "unusued quotas" and breakdown per flavor, however:
      * that's easier than [#10105](https://github.com/kubernetes-sigs/kueue/issues/10105) in that it doesn't touch individual workloads, or API surface
      * [#11141](https://github.com/kubernetes-sigs/kueue/pull/11141) introduced caching of remote CQs (& LQs) on the manager, so now the missing bits boil down to using [these fields](https://github.com/kubernetes-sigs/kueue/blob/1744552f93d8a886c7265dd81aee3198d79424b6/apis/kueue/v1beta2/clusterqueue_types.go#L362-L377) from the already-fetched remote CQ resources in the computations.
