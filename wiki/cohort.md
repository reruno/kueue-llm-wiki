...
Later work added a first-class `Cohort` CRD with an optional parent reference, so cohorts form a tree. Borrowing is evaluated bottom-up: a CQ first tries its parent cohort, then that cohort's parent, and so on, subject to each level's `resourceGroups` limits (source: issue-3644.md, source: issue-3583.md).

Hierarchical cohorts complicate preemption: the "cohort to reclaim from" is not obvious when multiple levels are in play. Issues like "Preemption only fetches candidates of ClusterQueue's parent Cohort" (source: issue-3583.md) and "Priority is not respected and causes non-deterministic preemption using hierarchical cohort with fair sharing" (source: issue-10131.md) track the edge cases.
...
- **Does** provide a target for [[preemption]] reclaim — when a CQ needs to admit a workload using its nominal quota that's currently lent out, Kueue preempts borrowers in the same cohort (source: issue-1874.md).
...
Fair-sharing and subtree borrowing across hierarchy levels had a bug where subtree borrowing was ignored during scheduling (source: issue-10615.md); that's the kind of subtle accounting issue that hierarchical cohorts introduce. Tests around cohort metrics reliability are also ongoing (source: issue-10195.md, source: issue-10417.md).
