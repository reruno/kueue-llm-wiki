...
Before they were distinguished in condition messages and metrics, all preemptions said the same generic thing — which made diagnosis painful (source: issue-1874.md). They are now separate reasons:
...
3. **Preemption while borrowing (`borrowWithinCohort`).** A borrower in CQ-A wants to displace a borrower in CQ-B that is below a priority threshold. Policy: `spec.preemption.borrowWithinCohort.policy` (`Never` | `LowerPriority`), with `maxPriorityThreshold` setting the cap. `LowerPriorityBorrowersOnly` was added to protect nominal-quota workloads from this path (source: issue-10171.md).
...
For TAS, preemption must free not just aggregate quota but a specific topology domain — this is more constrained (source: issue-10497.md — TAS preemption can under-select candidates needed to free a topology domain). See [[topology-aware-scheduling]].
