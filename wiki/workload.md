...
- `spec.priorityClassName` / `spec.priorityClassSource` / `spec.priority` — see [[workload-priority]]. `priorityClassSource` distinguishes a stock `scheduling.k8s.io/PriorityClass` from a `kueue.x-k8s.io/WorkloadPriorityClass` (source: issue-7342.md).
...
There was historical discussion about letting users supply a Workload directly via PodTemplate references (source: issue-1004.md), but it was closed as not-planned — integration controllers remain the canonical authors.
...
The `PodSetAssignments` in `status.admission.podSetAssignments[]` mirrors the PodSet array and records the flavor chosen per PodSet (source: issue-1163.md). PodSetAssignments count must equal PodSet count.
...
A Workload's priority is derived once, at creation, from either a stock PriorityClass or a WorkloadPriorityClass. It's stored as an int32 in `spec.priority`. Priority is traditionally immutable after creation (source: issue-2593.md — why PriorityClassName is immutable while priority integer is mutable) though in-place mutation of priority class has been added and then expanded (source: issue-5004.md, source: issue-7138.md). See [[workload-priority]].

## Eviction, requeueing, deactivation

When an admitted Workload is preempted, or when [[gang-scheduling]]'s WaitForPodsReady timeout fires, the Workload is evicted: `status.conditions.Evicted=True`, `status.admission` cleared, job re-suspended. If `requeuingStrategy` permits, the Workload is re-added to the queue with a backoff. After too many evictions it can be deactivated (source: issue-2174.md). If `spec.active` is false, the Workload stays dormant.

## Finalizer and GC

A Workload carries a finalizer so quota accounting stays consistent while the underlying job is being torn down. Cases where the finalizer wasn't removed (plain Pods, PodGroups) have historically blocked GC (source: issue-6919.md).
